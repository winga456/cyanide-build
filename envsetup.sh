...
function setpaths()
{
    ...
    targetgcckernelversion=$(get_build_var TARGET_CYANIDE_ARM)
    ...
    export TARGET_CYANIDE_ROM=$(get_build_var TARGET_CYANIDE_ROM)
    export TARGET_NDK_CYANIDE_ROM=$(get_build_var TARGET_NDK_CYANIDE_ROM)
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                cyanide|otapackage|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
